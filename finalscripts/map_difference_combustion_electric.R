...
# df used for first prototype, but this df doesn't have other countries,
# like Poland, Czechia, Romania etc.)

missing_vehicles <- missing_vehicles %>% 
  filter(Entity %in% c("Austria", "UK", "Greece")) %>%
  group_by(Entity) %>% 
  mutate(all_number = sum(battery_electric_number, plugin_hybrid_number,
